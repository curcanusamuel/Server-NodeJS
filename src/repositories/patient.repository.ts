...
export const patientRepository = {
	async findAll(): Promise<Patient[]> {
		const result = await db.query(`SELECT * FROM patients WHERE deleted_at IS NULL ${ORDER_BY}`)
	...
	async list(params: PatientListParams = {}): Promise<PaginatedPatientsResult> {
		...
		const whereWithCursor = whereClause
			...
			: cursorClause
				? `WHERE deleted_at IS NULL AND ${cursorClause.replace(/^AND /, '')}`
  ...
  `
   ...
   ${filter}`
	...
	async findById(id: string): Promise<Patient | null> {
		const result = await db.query('SELECT * FROM patients WHERE id = $1 AND deleted_at IS NULL', [id])
		return result.rows[0] ? rowToPatient(result.rows[0]) : null
	},

	async findByCNP(codCNP: string): Promise<Patient | null> {
		const result = await db.query('SELECT * FROM patients WHERE cod_cnp = $1 AND deleted_at IS NULL', [codCNP])
		return result.rows[0] ? rowToPatient(result.rows[0]) : null
	},
	...
	async search(query: string): Promise<Patient[]> {
		...
		const result = await db.query(`SELECT * FROM patients ${whereClause} AND deleted_at IS NULL ${ORDER_BY}`, values)
		return result.rows.map(rowToPatient)
	},
	...
	async findFirstNavigation(query?: string): Promise<PatientNavigationResult> {
		...
		// Build a filter that always includes deleted_at IS NULL
		const filter = whereClause
			? `${whereClause} AND deleted_at IS NULL`
			: 'WHERE deleted_at IS NULL'
       ...
       p.*,
       NULL AS previous_id,
       (SELECT id FROM patients ${filter} ${ORDER_BY} OFFSET 1 LIMIT 1) AS next_id,
	...
	async findNavigationById(id: string, query?: string): Promise<PatientNavigationResult | null> {
		...
		const filterAnd = whereClause
			? `${whereClause} AND deleted_at IS NULL AND`
       ...
       SELECT * FROM patients WHERE id = $${idIndex} AND deleted_at IS NULL
     )
     SELECT
       cp.*,
	...
	async update(id: string, data: UpdatePatientInput): Promise<PatientUpdateResult> {
		...
		// If client sent version, check it
       ...
       SET ${fields.join(', ')}
       WHERE id = $${idIndex} AND is_verified = false AND deleted_at IS NULL ${versionCheck}
       RETURNING *`,
			values
		)
		...
		const statusResult = await db.query(
			'SELECT is_verified, deleted_at, version FROM patients WHERE id = $1',
			[id]
		)
		if (!statusResult.rows[0]) return { status: 'not_found' }
		if (statusResult.rows[0].deleted_at) return { status: 'not_found' }
		if (statusResult.rows[0].is_verified) return { status: 'verified' }
	...
	async updateVerification(id: string, isVerified: boolean): Promise<Patient | null> {
		const result = await db.query(
			'UPDATE public.patients SET is_verified = $1 WHERE id = $2 AND deleted_at IS NULL RETURNING *',
			[isVerified, id]
		)
		return result.rows[0] ? rowToPatient(result.rows[0]) : null
	},
